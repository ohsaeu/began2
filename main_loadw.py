...
def loadWeight(sess, npz_path, npz_itr, ckpt_meta, target):
    
    g_params = np.load( npz_path+npz_itr+'net_g.npz' )['params']
    d_params = np.load( npz_path+npz_itr+'net_d.npz' )['params']
        ...
        if target=='D':
            with tf.variable_scope(scope[0]) as vs1:
                ...
                with tf.variable_scope(scope[1]) as vs2:
                    ...
                    if scope[0]=='encoder' and e_idx<len(e_params):
...
def main():
    ...
    save_npz(g_vars, name=os.path.join(checkpoint_dir, 'final_net_g.npz'), sess=sess)
    save_npz(enc_vars, name=os.path.join(checkpoint_dir, 'final_net_g.npz'), sess=sess)
    save_npz(dec_vars, name=os.path.join(checkpoint_dir, 'final_net_g.npz'), sess=sess)          
    saver.save(sess, checkpoint_dir+"began2_model.ckpt")  
